...
from scripts.utils import (
    ...
    find_data_files,
    parse_txt_annotations,
    build_image_index,
    _normalize_label_name,
    infer_severity_from_name,
    get_disease_details,
    get_crop_type,
    disease_to_severity,
    find_public_data_files,
)
...
def train_multitask_with_synergy(sample_ratio=0.5, num_epochs=15, early_stopping_patience=5, data_dir=None, dataset_mode="mock"):
    ...
    # 2. 数据路径查找与加载
    if dataset_mode in {"public", "plantvillage", "kaggle"}:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-mode",
        type=str,
        choices=["mock", "public", "plantvillage", "kaggle"],
        default="mock",
        help="数据模式：mock 使用当前赛题模拟数据，public 使用通用公开数据集文件夹，plantvillage 使用 PlantVillage 数据集，kaggle 与 public 相同"
    )
